...
CtlFormula::CtlFormula(string expression, State state, KripkeStructure& kripke)
{
    ...
    _kripke = kripke;
    _state = state;
    ConvertAsSysForm(expression); //parse the input Ctl check formula
	removeAllSpace(expression);
	_expression = expression;
}

void CtlFormula::ConvertAsSysForm(string& expression)
{
...
list<State> CtlFormula::SAT(string expression)
{
    ...
    case Atomic:
    {
        ...
        {
            if (checkListContainString(iter_atomic->Atom, leftExpression))
            {
    ...
    case Not:
    {
        ...
        {
            if (checkListContainState(states, *iter_f1States))
            {
                removeFromList(states, *iter_f1States);
            }
        }
        break;
    }
    case And:
    {
        ...
        {
            if (checkListContainState(andF2States, *iter_and))
            {
    ...
    case Or:
    {
        ...
        {
            if (!checkListContainState(states, *iter_or))
            {
...
list<State> CtlFormula::SAT_EU(string leftExpression, string rightExpression)
{
    ...
    while (!arelistStatesSame(x, y))
    {
        ...
        {
            if (checkListContainState(preEStates, *iter_state_w))
            {
        ...
        {
            if (!checkListContainState(newY, *iter_wAndPreE))
            {
...
list<State> CtlFormula::SAT_AF(string expression)
{
    ...
    while (!arelistStatesSame(x, y))
    {
        ...
        {
            if (!checkListContainState(newY, *iter_preAStates))
            {
...
list<State> CtlFormula::PreE(list<State>& y)
{
    ...
    for (iter_sourceState = _kripke.States.begin(); iter_sourceState != _kripke.States.end(); iter_sourceState++)
    {
        ...
        for (iter_destState = y.begin(); iter_destState != y.end(); iter_destState++)
        {
            ...
            if (checkListContainTransition(_kripke.Transitions, myTransition))
            {
                if (!checkListContainState(states, *iter_sourceState))
                {
...
list<State> CtlFormula::PreA(list<State>& y)
{
    ...
    {
        if (checkListContainState(S_Minus_Y, *iter_y))
        {
            removeFromList(S_Minus_Y, *iter_y);
        }
    }
    ...
    {
        if (checkListContainState(PreEY, *iter_PreE_S_Minus_Y))
        {
            removeFromList(PreEY, *iter_PreE_S_Minus_Y);
        }
    }
...
bool CtlFormula::arelistStatesSame(list<State>& list1, list<State>& list2)
{
    ...
    {
        if (!checkListContainState(list2, *iter_list1))
        {
            return false;
        }
    }
...
{
    if (checkListContainString(_kripke.Atoms, expression))
        return true;
    return false;
}
...
bool CtlFormula::checkListContainState(list<State>& dest, State src)
{
	return std::find(dest.begin(), dest.end(), src) != dest.end() ? true : false;
}

void CtlFormula::removeFromList(list<State>& dest, State src)
{
	std::remove(dest.begin(), dest.end(), src);
}

bool CtlFormula::checkListContainTransition(list<Transition>& dest, Transition src)
{
	return std::find(dest.begin(), dest.end(), src) != dest.end() ? true : false;
}

bool CtlFormula::checkListContainString(list<string>& dest, string src)
{
	return std::find(dest.begin(), dest.end(), src) != dest.end() ? true : false;
}

void CtlFormula::removeAllSpace(string& expression)
{
